...
const preferenceChoice = source("components/PreferenceChoice.qml")
const preferenceCategory = source("components/PreferenceCategoryButton.qml")
const preferenceToggle = source("components/PreferenceToggle.qml")
...
test("vertical gestures own overlays and never reserve center height", () => {
  ...
  assert.match(deck, /openOverlayName === "preferences"/)
...
test("Command Center opens a full-surface Preferences overlay", () => {
  assert.match(commandCenter, /label: "Preferences"[\s\S]*openOverlay\("preferences"\)/)
  assert.match(service, /\["notifications", "overview", "preferences"\]/)
  assert.match(deck, /\["", "notifications", "overview", "preferences"\]/)
  assert.match(deck, /overlayId: "preferences"[\s\S]*PreferencesModule\s*\{/)
  assert.match(preferences, /property string selectedCategory: "omadeck"/)
  ...
  assert.match(preferenceAction, /MouseArea\s*\{[\s\S]*preventStealing:\s*false/)
  assert.match(preferenceCategory, /root\.selected \? 1 : 0/)
  assert.match(preferenceToggle, /root\.checked \? Color\.accent : Color\.muted/)
})
